...
describe('request source', function()
  ...
  it('can jump to frame if source needs to be fetched from nested session #only', function()
    ...
    run_and_wait_until_initialized(config, srv2)
    srv2.spy.clear()
    wait_for_response(srv2, 'source')
    assert.are.same('source', srv2.spy.responses[3].command)
    assert.are.same('foobar', srv2.spy.responses[3].body.content)
    wait(function()
      return 'foobar' == api.nvim_buf_get_lines(0, 0, -1, false)[1]
    end)
    local lines = api.nvim_buf_get_lines(0, 0, -1, false)
